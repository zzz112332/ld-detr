...
def build_position_encoding(hidden_dim, position_embedding_arg, max_q_l, input_dropout=0.5):
    N_steps = hidden_dim
    if position_embedding_arg in ('v2', 'sine'):
        # TODO find a better way of exposing other arguments
        position_embedding = PositionEmbeddingSine(N_steps, normalize=True)
    # elif args.position_embedding in ('v3', 'learned'):
    #     position_embedding = PositionEmbeddingLearned(N_steps)
    else:
        raise ValueError(f"not supported {position_embedding_arg}")

    txt_pos_embed = TrainablePositionalEncoding(
            max_position_embeddings=max_q_l,
            hidden_size=hidden_dim, dropout=input_dropout)
