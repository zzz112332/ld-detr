...
from ld_detr.config import BaseOptions
...
from ld_detr.inference import eval_epoch, start_inference, setup_model
...
def train_epoch(model, criterion, train_loader, optimizer, opt, epoch_i,
                tb_writer):
    logger.info(
        f"[Epoch {epoch_i+1}], lr={optimizer.param_groups[-1]['lr']:.10f}")
    model.train()
    criterion.train()
    ...
    # print/add logs
    tb_writer.add_scalar("Train/lr", float(optimizer.param_groups[0]["lr"]),
                         epoch_i + 1)
...
def train(model, criterion, optimizer, lr_scheduler, train_dataset,
    ...
    # model.transformer.memory.init(model, train_loader, device=opt.device)
    ...
    for epoch_i in trange(start_epoch, opt.n_epoch, desc="Epoch"):
        ...
        if (epoch_i + 1) % save_interval == 0:  # additional copies
            ...
            torch.save(
                checkpoint,
                opt.ckpt_filepath.replace(".ckpt", f"_e{epoch_i:04d}.ckpt"))
