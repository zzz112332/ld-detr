...
from ld_detr.config import TestOptions
from ld_detr.model import build_model
from ld_detr.span_utils import span_cxw_to_xx
from ld_detr.start_end_dataset import StartEndDataset, start_end_collate, prepare_batch_inputs
from ld_detr.start_end_dataset_audio import StartEndDataset_audio, start_end_collate_audio, prepare_batch_inputs_audio
from ld_detr.postprocessing_detr import PostProcessorDETR
...
def compute_mr_results(model,
    ...
    for batch in tqdm(eval_loader, desc="compute st ed scores"):
        ...
        outputs = model(**model_inputs)
        prob = F.softmax(outputs["pred_logits"],
                         -1)  # (batch_size, #queries, #classes=2)
        if opt.span_loss_type == "l1":
            scores = prob[
                ...,
                0]  # * (batch_size, #queries)  foreground label is 0, we directly take it
            pred_spans = outputs["pred_spans"]  # (bsz, #queries, 2)
            _saliency_scores = outputs["saliency_scores"].half()  # (bsz, L)
            saliency_scores = []
            valid_vid_lengths = model_inputs["src_vid_mask"].sum(
                1).cpu().tolist()
            for j in range(len(valid_vid_lengths)):
                saliency_scores.append(
                    _saliency_scores[j, :int(valid_vid_lengths[j])].tolist())
        else:
        ...
        for idx, (meta, spans, score) in enumerate(
            ...
            cur_ranked_preds = torch.cat([spans, score[:, None]],
                                         dim=1).tolist()
            if not opt.no_sort_results:
                cur_ranked_preds = sorted(cur_ranked_preds,
                                          key=lambda x: x[2],
                                          reverse=True)
        ...
        if criterion:
            ...
            for k, v in loss_dict.items():
                loss_meters[k].update(
                    float(v) *
                    weight_dict[k] if k in weight_dict else float(v))
...
def eval_epoch(
    ...
    if opt.dset_name in ["tvsum", "youtube_uni"]:
        ...
        if opt.no_sort_results:
            save_submission_filename = save_submission_filename.replace(
                ".jsonl", "_unsorted.jsonl")
