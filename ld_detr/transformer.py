...
class Transformer(nn.Module):
    ...
    def forward(self, src, mask, query_embed, pos_embed, video_length=None):
        ...
        refpoint_embed = query_embed.unsqueeze(1).repeat(
            1, bs, 1)  # (#queries, batch_size, d)

        memory = self.encoder(src, src_key_padding_mask=mask,
                              pos=pos_embed)  # (L, batch_size, d)
    ...
    def forward(
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            if self.modulate_t_attn:
                ...
                query_sine_embed *= (reft_cond[..., 0] /
                                     obj_center[..., 1]).unsqueeze(-1)
...
class T2V_TransformerEncoderLayer_no_global(nn.Module):
    ...
    def forward_post(
        ...
        q, k, v = pos_src[:video_length], pos_src[video_length:], src[
            video_length:]
        ...
        qmask, kmask = src_key_padding_mask[:, :video_length].unsqueeze(
            2), src_key_padding_mask[:, video_length:].unsqueeze(1)
        attn_mask = (torch.matmul(qmask.float(), kmask.float()).bool().repeat(
            self.nhead, 1, 1))
    ...
    def forward_pre(
        ...
        src2 = self.self_attn(
            ...
            k,
            value=v,
            attn_mask=src_key_padding_mask[:, :75].permute(1, 0),
            key_padding_mask=src_key_padding_mask[:, 75:],
        )[0]
...
class T2V_TransformerEncoderLayer(nn.Module):
    ...
    def forward_post(
        ...
        qmask, kmask = src_key_padding_mask[:, 1:video_length + 1].unsqueeze(
            2), src_key_padding_mask[:, video_length + 1:].unsqueeze(1)
        attn_mask = (torch.matmul(qmask.float(), kmask.float()).bool().repeat(
            self.nhead, 1, 1))
        # print(attn_mask.shape)
        # print(attn_mask[0][0])
